...
module.exports = (grunt) => {
  ...
  grunt.initConfig({
    ...
    concat: {
      license: {
        ...
        files: {
          'dist/shan-enhanced-lists/plugin.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ],
          'dist/shan-enhanced-lists/plugin.min.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ]
        }
    ...
    copy: {
      css: {
        files: [
          { src: [ 'CHANGELOG.txt', 'LICENSE.txt' ], dest: 'dist/shan-enhanced-lists', expand: true }
        ]
      }
    },
  ...
  grunt.registerTask('version', 'Creates a version file', () => {
    grunt.file.write('dist/shan-enhanced-lists/version.txt', BUILD_VERSION);
  });
